...
namespace GroupDocs.Annotation.MVC.Products.Annotation.Controllers
{
    ...
    public class AnnotationApiController : ApiController
    {
        ...
        /// </summary>
        /// <param name="fileTreeRequest">AnnotationPostedDataEntity.</param>
        ...
        public HttpResponseMessage LoadDocumentDescription(AnnotationPostedDataEntity loadDocumentRequest)
        {
            ...
            try
            {
                ...
                string documentPath = this.GetDocumentPath(documentGuid);
                ...
                if (this.supportedImageFormats.Contains(fileExtension))
                {
                    documentType = "image";
                }
                else if (this.supportedDiagrammFormats.Contains(fileExtension))
                {
                    documentType = "diagram";
                }

                // check if document contains annotations
                AnnotationInfo[] annotations = this.GetAnnotations(documentPath, documentType, password);
        ...
        /// Get document page.
        /// </summary>
        /// <param name="loadDocumentPageRequest">AnnotationPostedDataEntity.</param>
        ...
        /// Download document.
        /// </summary>
        /// <param name="annotateDocumentRequest">AnnotationPostedDataEntity.</param>
        /// <returns>HttpResponseMessage.</returns>
        [HttpPost]
        ...
        public HttpResponseMessage DownloadAnnotated(AnnotationPostedDataEntity annotateDocumentRequest)
        {
            ...
            {
                // set exception message
                return this.Request.CreateResponse(HttpStatusCode.InternalServerError, new Resources().GenerateException(new ArgumentNullException("Annotations data is empty")));
            }
            ...
            try
            {
                Stream inputStream = this.AnnotateByStream(annotateDocumentRequest);
                HttpResponseMessage response = new HttpResponseMessage(HttpStatusCode.OK)
                {
        ...
        /// Annotate document.
        /// </summary>
        /// <param name="annotateDocumentRequest">Annotation posted data entity.</param>
        ...
        public HttpResponseMessage Annotate(AnnotationPostedDataEntity annotateDocumentRequest)
        {
            ...
            {
                // get/set parameters
                string documentGuid = this.GetDocumentPath(annotateDocumentRequest.guid);
                ...
                string notSupportedMessage = string.Empty;
                ...
                if (annotateDocumentRequest.print)
                {
                    annotatedDocument.pages = this.GetAnnotatedPagesForPrint(documentGuid);
            ...
            return this.Request.CreateResponse(HttpStatusCode.OK, annotatedDocument);
        }

        private static void RemoveAnnotations(string path)
        {
            try
            {
        ...
        private static Stream GetCleanDocumentStream(string path)
        {
            try
            {
                Stream resultStream = null;
